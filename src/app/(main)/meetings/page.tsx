    ...
    const fetchMeetings = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          const mappedMeetings = response.data.map((meeting: { _id: string; time: string; status: string; fullName: string; email: string }) => ({
            id: meeting._id,
            time: meeting.time,
              ...
              {meetings.length === 0 && activeTab === "action" ? (
                  ...
                  <Table.Td colSpan={5} className="py-16">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <Button
                        variant="filled"
                        size="md"
                        className="font-normal radius-lg"
                      >
                ...
                meetings.map((meeting, idx) => (
                  <Table.Tr key={meeting.id || idx} className="hover:bg-[#F2F0FF]">
                    ...
                    <Table.Td>
                      <span className="px-4 py-1 bg-[#9DE1BE] text-black rounded-lg text-sm font-semibold"
                      style={{

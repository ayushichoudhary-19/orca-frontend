...
import RejectedRepsTable from "@/components/SalesRep/RejectedRepsTable";
...
export default function RejectedRepsPage() {
  ...
  useEffect(() => {
    const fetchReps = async () => {
      try {
        const res = await axiosClient.get(`/api/auditions/${campaignId}/reps?status=rejected`);
        setReps(res.data);
      } catch (err) {
        console.error("Error fetching rejected reps", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Container size="xl" className="py-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Rejected Reps</h1>
        <Button
          size="md"
          radius="md"
